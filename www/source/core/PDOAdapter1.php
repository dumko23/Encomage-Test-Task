...
class PDOAdapter1
{
    ...
    public static function getFromDBComments($postId): bool|array
    {
        $queryGet = 'select postId, userName, text, date from PostList.Comments where postID=' . $postId .';';
        return static::db()->query($queryGet)->fetchAll();

    }

    public static function insertToDBComments($postId, string $userName, string $text): void
    {
        static::db()->prepare('insert into PostList.Comments (postId, userName, text)
                                values (?, ?, ?)')->execute([$postId, $userName, $text]);
    }
